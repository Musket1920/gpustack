...
class VLLMServer(InferenceServer):
    ...
    def _preflight_direct_process(
        ...
        failures: List[str] = []
        executable = command_args[0] if command_args else "vllm"
        ...
        if failures:
            ...
            logger.error(
                "Direct-process vLLM preflight failed for %s: %s",
                self._model_instance.name,
                message,
            )
            raise RuntimeError(
                f"Direct-process vLLM host prerequisites not met: {message}"
            )
    ...
    def _check_direct_process_command(
        ...
        if importlib.util.find_spec("vllm") is not None:
            failures.append(
                f"`{executable}` is not available on PATH. The `vllm` package is installed, but the CLI entrypoint is missing for direct-process launch"
            )
            return

        failures.append(
            f"`{executable}` is not available on PATH and the `vllm` package is not importable"
        )
    ...
    def _build_command_args(
        ...
        """
        Build vLLM command arguments for serving execution.
        """
        arguments = [
            "vllm",
            "serve",
