...
class LlamaCppServer(CustomServer):
    _DIRECT_PROCESS_BOOTSTRAP_RECIPE_ID = "llama.cpp"
    ...
    def _start_direct_process(
        ...
        if (
            ...
            raise ValueError(
                "Direct-process llama.cpp does not support distributed launches; only single-worker direct-process is implemented."
            )
        ...
        env = self.merge_direct_process_prepared_env_artifacts(
            self.build_direct_process_env(),
            launch_artifacts,
        )
        unwrapped_command_args = self.build_direct_process_command(port=port)
